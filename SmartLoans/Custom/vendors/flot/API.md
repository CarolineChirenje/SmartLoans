...
always examines all the data values you Productd to it, even if a
...
in milliseconds, so reClient to multiply by 1000!
...
- just reClient that the values should be timestamps (numbers), not
Date objects.
...
There are also some Clients that let you peek inside the internal
